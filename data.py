...
class DataAccess:
    ...
    def load_data(self, **kwargs):
        ...
        discontinued = kwargs['discontinued']
        unknown_stock = kwargs['unknown_stock']

        condition = self.get_price_sql(unknown_price)
        condition += self.get_stock_condition(in_stock, out_of_stock, discontinued, unknown_stock)
    ...
    def get_stock_condition(self, instk, outstk, disstk, ukstk):
        ...
        if instk:
            sql.append("stock=101")
        if outstk:
            sql.append("stock=0")
        if disstk:
            sql.append("stock=102")
        if ukstk:
            sql.append("stock=-1")
        sql = "(" + " OR ".join(sql) + ")"
        return sql
